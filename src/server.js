...
const { createPaymentJobService } = require('./services/payment-job-service');
...
async function main() {
  ...
  const jobs = createPaymentJobService({ db, config, providers, outbox });
  const paymentService = createPaymentService({ db, config, providers, outbox, jobs });
  jobs.bindPaymentService(paymentService);
